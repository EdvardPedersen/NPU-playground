...
def my_memcpy(dev, image_width, image_height, num_columns, bypass):
    ...
    # The cores produce vertical column strips, so the natural device layout is
    # column-major: bufOut[col*H + row].  Drain it as one contiguous burst per
    # NPU column (innermost stride 1) -- this keeps the shim DMA fast.  The
    # column-major -> row-major transpose is done for free on the GPU at blit
    # time (90deg rotation + flip in the host), so it never touches memory here.
    taps_out = [
        TensorAccessPattern(
            (1, size),
            i * chunk_per_col,
